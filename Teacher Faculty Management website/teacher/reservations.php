...
require_once $_SERVER['DOCUMENT_ROOT'] . '\Teacher Faculty Management website\classes\Reservation.php';
                ...
                <div class="col-md-4">
                    ...
                    <select id="time_slot_id" name="time_slot_id" class="form-select" required>
                        ...
                        <?php while ($slot = $timeSlots->fetch_assoc()): ?>
                            <option value="<?php echo $slot['id']; ?>"><?php echo htmlspecialchars($slot['slot_name'] . ' - ' . substr($slot['start_time'],0,5) . ' to ' . substr($slot['end_time'],0,5)); ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div class="col-md-4">
                    ...
                    <select id="classroom_id" name="classroom_id" class="form-select" required>
                        ...
                        <?php while ($r = $rooms->fetch_assoc()): ?>
                            <option value="<?php echo $r['id']; ?>"><?php echo htmlspecialchars($r['room_number'] . ' - ' . $r['room_name'] . ' (' . $r['room_type'] . ')'); ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                        ...
                        <?php if ($myReservations && $myReservations->num_rows > 0): ?>
                            <?php while ($resv = $myReservations->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($resv['room_number'] . ' - ' . $resv['room_name']); ?></td>
