    ...
    if (empty($email)) {
        ...
        if (empty($error)) {
            ...
            if (!empty($password)) {
                ...
                $update = $conn->prepare("UPDATE users SET email = ?, profile_image = ?, password = ? WHERE id = ?");
                $update->bind_param('sssi', $email, $profile_image, $hashed, $user_id);
            } else {
                $update = $conn->prepare("UPDATE users SET email = ?, profile_image = ? WHERE id = ?");
                $update->bind_param('ssi', $email, $profile_image, $user_id);
            }
    ...
    <nav class="navbar navbar-expand-lg navbar-dark">
        <div class="container-fluid">
            <a class="navbar-brand" href="dashboard.php">
                <i class="fas fa-graduation-cap"></i> TFMS
            </a>
