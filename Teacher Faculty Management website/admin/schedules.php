    ...
    if ($conflictRow['count'] > 0) {
        ...
        if ($availability && strtolower($availability['status']) === 'unavailable') {
            ...
            if ($roomConflictRow) {
                ...
                $timeSlotQuery = "SELECT slot_name, start_time, end_time FROM time_slots WHERE id = ?";
        ...
        <div>
            <?php if ($action === 'view_schedule' && $viewTeacher): ?>
                <h1>
                    ...
                    <?php echo htmlspecialchars($viewTeacher['first_name'] . ' ' . $viewTeacher['last_name']); ?>'s Schedule
                </h1>
...
<?php if ($action === 'view_schedule' && $viewTeacher): ?>
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5><?php echo htmlspecialchars($viewTeacher['first_name'] . ' ' . $viewTeacher['last_name']); ?> - Weekly Schedule</h5>
                        ...
                        <tr>
                            <td class="time-slot">
                                <div class="text-center">
                                    ...
                                    <small class="text-muted"><?php echo date('g:i A', strtotime($timeSlot['end_time'])); ?></small>
                                </div>
                            </td>
                            <?php foreach ($days as $day): ?>
                            <td class="schedule-cell">
                                ...
                                if (isset($scheduleData[$day][$timeSlot['id']])) {
                                    ...
                                    <div class="class-block editable-block" 
                                         data-schedule-id="<?php echo $class['id']; ?>"
                                         title="Click to edit this schedule"
                                         style="cursor: pointer;">
                                        <div class="subject-code"><strong><?php echo htmlspecialchars($class['subject_code'] ?? 'N/A'); ?></strong></div>
                                        <div class="subject-name"><?php echo htmlspecialchars($class['subject_name']); ?></div>
                                        <div class="room-info">
                                            <small><i class="fas fa-map-marker-alt"></i> <?php echo htmlspecialchars($class['room_number']); ?></small>
                                        </div>
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <div class="position-relative">
                        <input type="text" id="searchInput" class="form-control" placeholder="Type to search by name, email, or department..." autocomplete="off">
                        ...
                        if ($result && $result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                <tr data-name="<?= strtolower(htmlspecialchars($row['first_name'] . ' ' . $row['last_name'])) ?>" 
                                    ...
                                    <td>
                                        <span class="badge badge-<?php echo ($row['status'] === 'active') ? 'success' : 'warning'; ?>">
                                            <?php echo htmlspecialchars($row['status']); ?>
                                        </span>
                                    </td>
                                    <td>
                                        <a href="schedules.php?action=view_schedule&teacher_id=<?php echo $row['id']; ?>" class="btn btn-sm btn-primary">
